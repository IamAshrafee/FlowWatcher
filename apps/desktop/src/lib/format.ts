 ...
 */
export function thresholdToBytes(value: number, unit: 'KB/s' | 'MB/s'): number {
 ...
 */
export function durationToSeconds(value: number, unit: 'seconds' | 'minutes'): number {
 ...
 */
export function monitorModeLabel(mode: 'download_only' | 'upload_only' | 'both'): string {
  switch (mode) {
    case 'download_only':
      return 'Download';
    case 'upload_only':
