...
export function DashboardPage() {
  ...
  const { countdownState, startCountdown, cancelCountdown, executeNow, isCountdownActive } =
    useCountdown();
  ...
  return (
    <div className="animate-slide-up space-y-5">
      ...
      <button
        ...
        style={{
          backgroundColor: isMonitoring ? 'var(--color-surface)' : 'var(--color-accent)',
          color: isMonitoring ? 'var(--color-text-secondary)' : 'var(--color-text-inverse)',
          border: isMonitoring ? '1px solid var(--color-border-default)' : '1px solid transparent',
          cursor: 'pointer',
      ...
      {status.status !== 'Idle' && (
        <div
          ...
          style={{
            ...
            color: isMonitoring ? 'var(--color-accent)' : 'var(--color-warning)',
            border: isMonitoring
              ? '1px solid rgba(58, 186, 180, 0.15)'
              : '1px solid rgba(229, 115, 115, 0.15)',
          }}
        >
          {status.status === 'Monitoring' && t('dashboard.statusMonitoring')}
          {status.status === 'TriggerPending' && t('dashboard.statusTriggerPending')}
      ...
      {isMonitoring && (
        <button
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.backgroundColor = 'rgba(255, 183, 77, 0.12)';
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.backgroundColor = 'rgba(255, 183, 77, 0.06)';
          }}
        >
...
export function LogsPage() {
  ...
  // Fetch logs on mount and expose for refresh button.
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
      ...
      >
        ...
        <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
          {isLoading ? (
            ...
            <p className="p-6 text-center text-sm" style={{ color: 'var(--color-text-muted)' }}>
              {search ? t('logs.noMatch') : t('logs.emptyState')}
            </p>
          ) : (
            sorted.map((entry, i) => {
...
export function SettingsPage() {
  ...
  const { settings, updateSettings, loadSettings, resetDefaults } = useSettingsStore();
  ...
  return (
    <div className="animate-slide-up space-y-4">
          ...
          >
            {(['dark', 'light', 'auto'] as const).map((themeValue) => (
              <button
                ...
                style={{
                  backgroundColor:
                    settings.theme === themeValue ? 'var(--color-accent)' : 'transparent',
                  color: settings.theme === themeValue ? '#fff' : 'var(--color-text-secondary)',
                  cursor: 'pointer',
                  border: 'none',
                }}
              >
      ...
      <SettingsSection title={t('settings.behavior')}>
        ...
        <SettingsRow
          label={t('settings.keepScreenOnLabel')}
        ...
        <SettingsRow
          label={t('settings.activityLoggingLabel')}
          description={t('settings.activityLoggingDescription')}
        >
          <ToggleSwitch
        ...
        >
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) =>
                updateSettings({
                  pre_action_delay_mins: Math.max(0, Math.min(60, parseInt(e.target.value) || 0)),
                })
              }
