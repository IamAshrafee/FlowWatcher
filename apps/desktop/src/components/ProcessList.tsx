    ...
    <div className="space-y-3">
      ...
      >
        ...
        {!isLoading && filteredProcesses.length === 0 && (
          <p className="p-4 text-center text-sm" style={{ color: 'var(--color-text-muted)' }}>
            {searchQuery ? t('processList.noMatch') : t('processList.noProcesses')}
          </p>
        )}

        {filteredProcesses.map((proc) => {
          const isChecked = watchedProcesses.includes(proc.name);
          const { value, unit } = formatSpeed(proc.estimated_network_bytes);
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span
                    ...
                    style={{
                      color: isChecked ? 'var(--color-accent)' : 'var(--color-text-primary)',
                    }}
                  >
                    {proc.name}
                  </span>
                  {proc.is_suggested && (
                    <span
                      ...
                      style={{
                        backgroundColor: 'rgba(58, 186, 180, 0.12)',
                        color: 'var(--color-accent)',
                      }}
                    >
